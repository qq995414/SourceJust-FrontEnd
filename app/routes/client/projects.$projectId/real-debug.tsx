  ...
  return(
    <div className="w-full h-screen p-5">
      ...
      <div className="w-full mt-32 text-center">
        <Button
          className="w-32 h-12"
          onClick={()=>{setIsShowLink(true);}}>下一步</Button>
      </div>
      {isShowLink &&
        <div className="fixed top-0 bottom-0 left-0 right-0">
          ...
          rounded-xl shadow-xl flex flex-col items-center pt-20" >
            <p className="font-semibold text-lg">是否已確認正常運作了</p>
            <p className="font-semibold text-lg">下一步將開始計算維護保固</p>
            <Button
              onClick={() => { navigate('./../case-close'); }}
              className="w-1/4 text-sm font-medium mt-16 mb-6 h-10"
            >確認</Button>
            <Button
              onClick={() => { setIsShowLink(false); }}
              className="w-1/4 text-sm font-medium bg-usual-gray h-10"
            >取消</Button>
          </div>
        </div>}
    </div>
