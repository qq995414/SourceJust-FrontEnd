...
export default function Index() {
  ...
  const [coverImgState, setCoverImgState] = useState(true);
  const [smallImgState, setSmallImgState] = useState(true);
  ...
  const [description, setDescription] = useState({
    ...
    editorState: EditorState.createWithContent(
      ContentState.createFromText(sampleMarkup)
    )
  });
...
h-8 rounded-lg ml-3  font-semibold bg-Primary-3-Primary"
          ...
          <div className='w-2/12 blog-details h-screen px-8 pt-20 fixed' style={{ right: '0px' }}>
            ...
            <div className='grid pt-5 px-2'>
              <a>資料狀態</a>
              {isDisable == true && <div className='table-state-green flex w-4/12 pl-2'>
                <div className='table-state-green-bell mr-2'></div>
                <a>已發布</a>
              </div>}
              {isDisable == false && <div className='table-state-red flex w-4/12 pl-2'>
            ...
            />
            {isDisable == true && <button
              className="border-2  mt-8 w-10/12 text-white
h-8 rounded-lg ml-3  font-semibold bg-Primary-3-Primary blog-clear-btn"
            >
              已發佈</button>}
            {isDisable == false && <button
              className="border-2  mt-8 w-10/12 text-white
h-8 rounded-lg ml-3  font-semibold bg-Primary-3-Primary blog-clear-btn"
            >
              草稿</button>}
